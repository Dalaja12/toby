...
async function searchJioSaavn() {
    const query = document.getElementById('jiosaavnSearch').value.trim();
    if (!query) return;

    addMessage(`Buscando: "${query}"...`, 'bot');
    showTypingIndicator();

    try {
        // Usa un proxy para evitar CORS
        const proxyUrl = 'https://cors-anywhere.herokuapp.com/';
        const apiUrl = `https://www.jiosaavn.com/api.php?__call=autocomplete.get&query=${encodeURIComponent(query)}`;
        
        const response = await fetch(proxyUrl + apiUrl, {
            headers: {
                'X-Requested-With': 'XMLHttpRequest' // Necesario para algunos proxies
            }
        });
        
        const data = await response.json();
        console.log("Respuesta de JioSaavn:", data); // Verifica la estructura

        if (data.songs?.data?.length > 0) {
            const song = data.songs.data[0];
            const songId = song.id;

            // Obtener detalles de la canción (con proxy)
            const detailsUrl = `https://www.jiosaavn.com/api.php?__call=song.getDetails&_format=json&pids=${songId}`;
            const detailsResponse = await fetch(proxyUrl + detailsUrl);
            const detailsData = await detailsResponse.json();
            
            const mediaUrl = detailsData[songId].media_preview_url
            ...
            audio.play();
            addMessage(`🎵 Reproduciendo: "${song.song}"`, 'bot');
        } else {
            addMessage("No encontré resultados. Prueba con otro nombre.", 'bot');
        }
    } catch (error) {
        console.error("Error:", error);
        addMessage("Error al buscar. Prueba con /fma para música libre.", 'bot');
    }
}